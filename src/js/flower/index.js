import p5 from 'p5';
import { baseColor, mainConfig } from "./config";
...
const sketch = (s) => {
	...
	const { createCanvas, noFill, smooth, background, noLoop, resetMatrix, radians, translate, stroke, strokeWeight, line, scale, rotate, push, pop, width, height } = s;
	...
	function turtle() {
		background(bgc);
		const angle = radians(Math.random() * (mainConfig.stems.angle.max - mainConfig.stems.angle.min) + mainConfig.stems.angle.min);
		resetMatrix();
